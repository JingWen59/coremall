...
public class SysGeneratorService {
    @Autowired
    @Qualifier("getGeneratorDao")  // 指定使用 MySQL 的 GeneratorDao
    private GeneratorDao generatorDao;


    public PageUtils queryList(Query query) {
        // 将 Query 对象转为 Map（兼容原接口）
        Map<String, Object> params = new HashMap<>();
//        params.put("tableName", query.getTableName());
        params.put("tableName", query.get("tableName"));

        // 查询列表（不分页）
        List<Map<String, Object>> list = generatorDao.queryList(params);
        ...
        if (page > 0 && limit > 0 && list.size() > limit) {
